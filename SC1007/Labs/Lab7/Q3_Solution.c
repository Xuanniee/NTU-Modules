...
int main()
{
    ...
    // Initialise an Empty Chessboard
    ...
    // Count Number of Solutions for BSize Chessboard
    ...
    for (i = 0; i < BSize; i++) { // check row by row in the (col)^th column
        ...
        if (isSafe(board,BSize, i, col)) {
           ...
           // recursive the next column placement
            ...
            //Delete the queen - Backtrack; Always, the only difference
            board[i][col] = 0;
        }
    }
