...
int nQueensAll(int** board, int BSize, int col)
{
    ...
    // BSize is Dimensions of Board
    // Col is the current Column we are at
    int solution = 0;
    ...
    if (col >= (BSize-1))
    {
        ...
        {
            if (isSafe(board, BSize, j, col) == 1)
            {
                numSolutions += 1;
            }    
        }
        // Return True as Solution(s) is found
        return numSolutions;
    }

    // Iterate through all the Rows in the col-th Column
    for (int i = 0; i < BSize; i += 1)
    {
        if (isSafe(board, BSize, i, col) == 1)
        {
            // Safe; Assign the Current Position
            board[i][col] = 1;  

            // Recursively Check the Subsequent Columns to see if there is a solution
            solution += nQueensAll(board, BSize, (col+1));

            // Always Backtrack so that I can discover the other solutions as well
            board[i][col] = 0;
        }
    }
    count = solution;
