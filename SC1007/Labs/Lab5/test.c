...
ListNode* HashSearch(HashTable Q3Hash, int key)
{
    ...
    int iterable = Q3Hash.Table[hash_value].size;
    ListNode *cursor = Q3Hash.Table[hash_value].head;
    for (int i = 0; i < iterable; i += 1)
    {
        if (cursor->key == key)
        {
            return cursor;
...
{
    int hash_value = Hash(key, Q3HashPtr->hSize);
    ...
    newNode->key = key;
    newNode->next = Q3HashPtr->Table[hash_value].head;
    Q3HashPtr->Table[hash_value].head = newNode;

    Q3HashPtr->nSize += 1;
    Q3HashPtr->Table[hash_value].size += 1;
