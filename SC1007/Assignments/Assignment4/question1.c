...
int HashDelete(HashTable* Q3HashPtr, int key)
{
    ...
    {
        // Deletion unsuccessful as the Key is not present
        return 0;
    }

    int hTableSize = Q3HashPtr->hSize;
    int numInsertedKeys = Q3HashPtr->nSize;

    // Determine Index of Key
    int hash_value = Hash(key, hTableSize);

    // Create Pointers to Delete
    ListNode *cursor = NULL, *prev = NULL;

    // Find the Node
    cursor = Q3HashPtr->Table[hash_value].head;
    prev = cursor;

    while (cursor != NULL)
    {
        // First Node is Key
        if (Q3HashPtr->Table[hash_value].head->key == key)
        {
            // Remove the Key from Linked List
            Q3HashPtr->Table[hash_value].head = cursor->next;
        ...
        // Move to Next Node
        if (cursor != Q3HashPtr->Table[hash_value].head)
        {
            prev = prev->next;
