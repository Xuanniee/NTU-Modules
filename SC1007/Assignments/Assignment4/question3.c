...
void adjL2adjM(Graph *g){
    ...
    int **outerArray = (int **) malloc(sizeof(int*)*numVertices);
    int **cursor = outerArray;
    for (int i = 0; i < numVertices; i += 1)
    {
        // Create the inner linked list [Vertical Column]
        cursor[i] = calloc(numVertices, sizeof(int));   // Initialize Array of Zeroes
    }
    cursor = outerArray;
    // Graph test;
    // test.type = ADJ_MATRIX;
    // test.V = numVertices;
    // test.E = numEdges;
    // test.adj.matrix = outerArray;
    // printGraphMatrix(test);

    // Iterate through the Adjacency List
    ListNode **adj_list = g->adj.list;
    // Pointer to Adjacency List Head
    // ListNode *array = *adj_list;
    for (int i = 0; i < numVertices; i += 1)
    {
        // Pointer at LL at each Index
        ListNode *innerLL = *(adj_list + i);
        // Fill Array with 1s at Correct Indexes
        while(innerLL != NULL)
        {
            // Get the Index where there is an edge
            int **outerCursor = (cursor + i);
            int index = innerLL->vertex;

            // Set the Value to 1 where there is an Edge in ADJ_MATRIX
            *(*outerCursor + index - 1) = 1;

            innerLL = innerLL->next;
        }

    }
    g->type = ADJ_MATRIX;
    g->adj.matrix = outerArray;
